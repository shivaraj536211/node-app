        ...
        stage('Deploy to k8s'){
            steps{
                ...
                sshagent(['k8s-machine']) {
                    sh "scp -o StrictHostKeyChecking=no services.yml node-app-pod.yml ubuntu@172.31.37.102:/home/ubuntu/"
                    script{
                        try{
                            sh "ssh ubuntu@172.31.37.102 kubectl apply -f ."
                        }catch(error){
                            sh "ubuntu@172.31.37.102 kubectl create -f ."
                        }
                    }
                }
            }
